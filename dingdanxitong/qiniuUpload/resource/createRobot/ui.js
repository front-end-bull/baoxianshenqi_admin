/*global plupload */
/*global qiniu */
function FileProgress(file, targetID,flag) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        var progressSize = $("<td/>");
        progressSize.addClass("progressFileSize").text("压缩前："+fileSize);
        ...
        if(flag=='over'){
            var fileSize = plupload.formatSize(file.size).toUpperCase();
            $('#'+this.fileProgressID+' .progressFileSize').append('<br>压缩后：'+fileSize)
        }
        this.reset();
    }
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    // var imageInfo = Qiniu.imageInfo("http://7xq93s.com1.z0.glb.clouddn.com/"+res.key);
    // console.log("-图片宽度: ")
    ...
    if (res.url) {
        ...
        // if(imageInfo.width>720){
        //     var zoomScale = parseInt(720/imageInfo.width*100)
        //     // url += "?imageMogr2/thumbnail/!"+zoomScale+"p"
        //     link += "?imageMogr2/thumbnail/!"+zoomScale+"p"
        // }
        recordUrl(link)
        str = "<div><strong>Link:</strong><a href=" + link + " target='_blank' > " + link + "</a></div>" +
            "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    }
    ...
    var imageView = '?imageView2/2/w/100/h/100';
        ...
        $(img).on('load', function() {
            ...
            if (!(ie && ie <= 9)) {
                ...
                var infoInner = '<div>格式：<span class="origin-format">' + imageInfo.format + '</span></div>' +
                    '<div>原始宽度：<span class="orgin-width">' + imageInfo.width + 'px</span></div>' +
                    '<div>原始高度：<span class="origin-height">' + imageInfo.height + 'px</span></div>';
